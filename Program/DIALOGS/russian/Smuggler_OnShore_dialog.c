...
void ProcessDialogEvent()
{
	...
	case "First time":
		...
		{
			if (CheckAttribute(PChar, "quest.Transportation.active") && sti(PChar.quest.Transportation.active) == true) //если заплатил, то разговаривают, иначе посылают
			{
				...
				if (IsWithCompanions() || sti(PChar.ship.type) != SHIP_NOTUSED)
				{
				...
				if (CheckAttribute(PChar, "quest.Transportation.PatrolFight") && sti(PChar.quest.Transportation.PatrolFight) == true)
				{
					...
					PChar.quest.Transportation.PatrolFight = false;
				...
				refStore.dialog.CurrentNode = "Transportation_TalkStart";
				//поплыл, иначе RemoveTransportationSmugglers грохнет всю ветку
				PChar.quest.Transportation.ship = true;
				PChar.quest.Munity = ""; // признак выхода с палубы

				SetLaunchFrameFormParam("Прошло " + sti(Pchar.quest.Transportation.destination.days) + " дней." + NewStr() + "Палуба корабля контрабандистов.",
																"Reload_To_Location", 0.1, 5.0);
				bQuestCheckProcessFreeze = true;
				WaitDate("", 0, 0, sti(Pchar.quest.Transportation.destination.days), rand(20), 0);
				bQuestCheckProcessFreeze = false;
				MakeCloneShipDeck(refStore, true); // подмена палубы
				SetLaunchFrameReloadLocationParam("Ship_deck", "reload", "reload1", "Transportation_TalkOnDeck");
			...
			else
			{
				...
				{
					if (CheckAttribute(PChar, "quest.Transportation.PatrolFight"))
					{
						...
						dialog.Text = RandPhraseSimple("Спасибо за помощь. Мы не забудем этого. Отправляемся.", "Молодец! Отлично сражал" + GetSexPhrase("ся", "ась") + ". Идем.");
						ChangeContrabandRelation(PChar, 5);
						DeleteAttribute(PChar, "quest.Transportation.PatrolFight");
	...
	case "GenQuestKillContraband_1":
		...
		if (CheckAttribute(PChar, "quest.Transportation.active") && sti(PChar.quest.Transportation.active) == true)
		{
			Statistic_AddValue(PChar, "contr_TransportatationKill", 1);
