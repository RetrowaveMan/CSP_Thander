void ProcessDialogEvent()
{
	...
	case "Cabin_PersonSelect":
		...
		if (!bDisableMapEnter && IsWithCompanions())
		{
			link.l1 = "Компаньона.";
	...
	case "GenTravel_Main":
		...
		SetAnyReloadToLocation(pchar.quest.Transportation.destination.loc, pchar.quest.Transportation.destination.group, pchar.quest.Transportation.destination.locator, "", 0, 0, 0, 0);
		AddDialogExitQuest("AnyReloadToLocation");
		chrDisableReloadToLocation = false;

		CloseQuestHeader("Gen_SmugglersTransportation");
		setCharacterShipLocation(PChar, pchar.quest.Transportation.destination.loc);
