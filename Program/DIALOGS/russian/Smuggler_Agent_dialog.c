void ProcessDialogEvent()
{
	...
	if (CheckAttribute(pchar, "quest.Transportation.days") && GetQuestPastDayParam("Transportation") > sti(PChar.quest.Transportation.days))
	{
	...
	case "pirateStartQuest_Smuggler":
		if (!CheckAttribute(pchar, "quest.Transportation.active") && !CheckAttribute(Pchar, "quest.Contraband.active"))
		{
	...
	case "Meeting_3":
		...
		if (IsWithCompanions() && GetBaseHeroNation() != PIRATE)
		{
		...
		bOk = CheckAttribute(pchar, "quest.Transportation.active") && (sti(pchar.quest.Transportation.active) == true);
		...
		if (bOk)
		{
			if (GetQuestPastDayParam("Transportation") > sti(PChar.quest.Transportation.days))
			{ // просрочка
				DeleteAttribute(PChar, "quest.Transportation");
				CloseQuestHeader("Gen_SmugglersTransportation");
				bOk = false;
	...
	case "AntiPGGContra":
		...
		if (Smugglers_IsUnlimitedContra() || Smugglers_Contraband_ShipRequirements())
		{
			npchar.quest.trade_date = lastspeak_date;
			Pchar.quest.Contraband.CurrentPlace = SelectSmugglingLocation();
			Pchar.quest.Contraband.City = NPChar.city;
			...
			{
				if (Smugglers_Contraband_IsEnoughRelations(npchar))
				{
					Dialog.Text = "Я знаю, с тобой можно иметь дело. Мы будем ждать тебя в месте, называющемся " + GetConvertStr(Pchar.quest.Contraband.CurrentPlace, "LocLables.txt") + ".";
				}
				else
	...
	case "Trade": //торговля с контрами - Gregg
		...
		case 2:
			...
			if (CheckAttribute(ind, "groupID") && sti(GetItemPrice(npchar.itemtype)) < 100)
	...
	case "Transportation":
		...
		if (Smugglers_Transportation_Requirements())
		{
			if (Smugglers_IsTransportation()) // условия обговорены
			{
				if (Smugglers_Transportation_Payed()) // мы заплатили
				{
				...
				{
					if (GetQuestPastDayParam("Transportation") == sti(PChar.quest.Transportation.days)) // срок оплаты не истек
					{
						Dialog.Text = "Прин" + GetSexPhrase("ёс", "есла") + " деньги?";
						Link.l1 = "Да.";
						Link.l1.go = "Transportation_pay";
						Link.l3 = "Я передумал" + GetSexPhrase("", "а") + "....";
						Link.l3.go = "Transportation_abort";
						Link.l2 = "Нет пока.";
					}
					else
					{
						if (GetQuestPastDayParam("Transportation") < sti(PChar.quest.Transportation.days)) // пришли платить слишком рано
						{
							Dialog.Text = "Я тебе уже все сказал.";
							Link.l2 = "Точно.";
							Link.l1 = "Я передумал" + GetSexPhrase("", "а") + "....";
							Link.l1.go = "Transportation_abort";
						}
						else // срок оплаты истек
						{
			...
			else // обговариваем условия
			{
				if (Smugglers_Transportation_Available())
				{
					//бухта...
					pchar.quest.Transportation.CurrentPlace = SelectSmugglingLocation();
					aref arTmp;
					makearef(arTmp, pchar.quest.Transportation);
					SetSmugglersTransportationDestination(arTmp);
					//за сколько доставят
					pchar.quest.Transportation.price = (sti(PChar.rank) * 250 + (100 - nRel) * 10 + rand(30) * 20) + sti(arTmp.destination.days) * 100;

					if (Smugglers_Transportation_IsAgentApprove(npchar) 
									&& !Smugglers_IsContraband()
									&& Smugglers_IsGoodPlaceFor("Transportation"))
					{
						nDay = 1 + rand(3);
						SetNPCQuestDate(npchar, "Transportation_Talk");
						SaveCurrentQuestDateParam("Transportation");

						Dialog.Text = "Что ж, можем доставить тебя до места " + GetConvertStr(locations[FindLocation(pchar.quest.Transportation.destination.loc)].id, "LocLables.txt") + " близ " +
													XI_ConvertString("Colony" + pchar.quest.Transportation.destination + "Gen") + " за " + pchar.quest.Transportation.price + " золотых. Принесёшь деньги через " +
													FindRussianDaysString(nDay) + ". Корабль будет ждать тебя в месте под названием " +
													GetConvertStr(locations[FindLocation(Pchar.quest.Transportation.CurrentPlace)].id, "LocLables.txt") + " ровно сутки.";

						pchar.quest.Transportation.days = nDay;
						Link.l1 = "Хорошо, меня устраивает.";
						Link.l1.go = "Transportation_agree";
						Link.l2 = "Нет, спасибо.";
					}
					else
					{
		...
		else
		{
			...
			{
				Dialog.Text = "Нет, мы не повезём вас всех. Только одного.";
	...
	//ГГ согласен ехать
	case "Transportation_agree":
		ReOpenQuestHeader("Gen_SmugglersTransportation");
		AddQuestRecord("Gen_SmugglersTransportation", "1");
		AddQuestUserData("Gen_SmugglersTransportation", "sSex", GetSexPhrase("", "а"));
		AddQuestUserData("Gen_SmugglersTransportation", "sLoc", GetConvertStr(pchar.quest.Transportation.CurrentPlace, "LocLables.txt"));
		...
		AddQuestUserData("Gen_SmugglersTransportation", "sPlaceTo", XI_ConvertString("Colony" + pchar.quest.Transportation.destination + "Gen"));
		AddQuestUserData("Gen_SmugglersTransportation", "sDays", FindRussianDaysString(sti(pchar.quest.Transportation.days)));
		AddQuestUserData("Gen_SmugglersTransportation", "sPrice", pchar.quest.Transportation.price);

		//активируем квест
		pchar.quest.Transportation.active = true;
		pchar.quest.Transportation.GuardNation = npchar.nation;
		NextDiag.CurrentNode = NextDiag.TempNode;
		DialogExit();
		break;

	//ГГ согласен платить
	case "Transportation_pay":
		//денег хватает?
		Sum = sti(pchar.quest.Transportation.price);
		if (sti(pchar.money) >= Sum)
		{
			AddMoneyToCharacter(pchar, -1 * Sum);
			//ставим флаг оплаты
			pchar.quest.Transportation.payed = true;
			Dialog.Text = "Приятно иметь с тобой дело. Корабль ждёт, не опоздай.";
			Link.l1 = "Постараюсь.";
			AddQuestRecord("Gen_SmugglersTransportation", "2");
			AddQuestUserData("Gen_SmugglersTransportation", "sSex", GetSexPhrase("", "а"));

			//ставим контру.
			PlaceSmugglersOnShore(PChar.quest.Transportation.CurrentPlace);
			//корабль на волнах в бухте....
			Sum = sti(pchar.quest.Transportation.destination.days);
			...
			SetCharacterShipLocation(sld, PChar.quest.Transportation.CurrentPlace);

			SetTimerCondition("RemoveTransportationSmugglers", 0, 0, 1, false);
		}
		//нет, посылаем в сад...
		else
		{
