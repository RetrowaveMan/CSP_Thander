...
{
	if (questFadeOut != "")
		LAi_QuestDelay(questFadeOut, 0.55);
	if (questFadeIn != "")
		LAi_QuestDelay(questFadeIn, 1.10);
		...
		else
		{
			...
			// матрос с ЧЖ <--
			{
				...
				for (j = ItemDeadStartCount; j < ITEMS_QUANTITY; j++) // нужно оптимизировать!!! 137 - это много  начало с опред места
				{
					...
					{
						trace("itm name: " + name + " itm id: " + itm.id)
						...
						firstNum = rand(1000 - (GetCharacterSPECIALSimple(mchr, SPECIAL_L) * 20));
						secondNum = ((stf(typeRef.rare) + stf(typeRef.rare) * nLuck / 20.0) / makefloat(MOD_SKILL_ENEMY_RATE)) * 930.0;
						if (firstNum < secondNum)
						{
							value = sti(typeRef.min);
							value = value + rand(sti(typeRef.max) - value);
							itemID = itm.id;
							trace("itm.id = " + itemID + " typeRef.rare: " + typeRef.rare + " chances: " + firstNum + " < " + secondNum)
